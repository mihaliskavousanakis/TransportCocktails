...
% Fitting of porosity functional for non-adhering, non-releasing Liposomes
% The general form is: 
% phi (r) = x(1) * r^x(2) + x(3)
...
try
     ...
     % the liposome incubation concentration, 0.5 mM
     ...
     R=R/max(R); 
     % normalize the concentration with incubation concentration, 0.5
     C16 = C16/0.5; % measured concentration at 16 hrs uptake
     C24 = C24/0.5; % measured concentration at 24 hrs uptake
     C48= C48/0.5; % measured concentration at 48 hrs uptake
     % The Liposome concentration equilibrates after 16 hrs and we compute the
     % average concentration: 
     Cavg = (C16+C24+C48)/3;

     % Some concentration measurements are missing
     [Cavg,l] = rmmissing(Cavg);
     R=R(~l); 
